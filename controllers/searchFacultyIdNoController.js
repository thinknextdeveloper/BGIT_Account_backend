const { searchFacultyByIDNo } = require("../models/searchFacultyIdNoModel");

const findFacultyByIDNo = async (req, res) => {
  try {
    const { idNo, facultyId, college, collegeName, allColleges } = req.query;
    const searchId = idNo || facultyId;
    const selectedCollege = college || collegeName;
    const isAllColleges = allColleges === "true" || allColleges === true;

    console.log("📥 [Controller] SearchFacultyByIDNo Query Params:", {
      query: req.query,
      searchId,
      selectedCollege,
      isAllColleges,
    ...
    if (!searchId || String(searchId).trim() === "") {
      return res.status(400).json({ success: false, message: "Please specify Faculty ID No" });
    }
    ...
    const rows = await searchFacultyByIDNo({
      idNo: searchId,
    ...
    console.error("searchFacultyIdNo error:", err);
    return res.status(500).json({ success: false, message: err.message });
  }
};

module.exports = { findFacultyByIDNo };
